...
int main()
{
    ...
    while(flag)
    {
        ...
        if(choice==1)
		{
            ...
            print(index); //print an alert for the user that it has added an item to his cart
            strs = add_to_list(index); //adds the chosen item to the list so it can be printed later in checkout
		...
		{
			  if(strs.empty()==0) //if the user has items in his cart he cannot leave the program
			  {                  
		...
		{
			if(strs.empty()==0) //if the user wants to empty his order
			{ 
			    total = 0.0; //resets the amount
				strs.clear(); //resets the list
...
{	//adds the element in the list string for checkout using struct
	checkout menu[size] = {
        {"Espresso"},
        {"Latte"},
        {"Cappuccino"},
